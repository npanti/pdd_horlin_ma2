...
d=struct(...
    ...
    'messageLength',4*1024*6,...        %bits sent
...
%CFO
d.Ts = 1/d.bandwidth;
d.CFO = 0.2*pi/(d.subCarriers*d.Ts);
...
%CFO Added
frameR=zeros(d.signalLength,d.RXAntenna);
for i=1:d.RXAntenna
    frameR(:,i)=addCFO(frame(:,i));
end
...
for i=1:d.RXAntenna
    preambleRX(:,i)=frameR(1:d.preambleLength,i);
    dataRX(:,i)=frameR(d.preambleLength+1:end,i);
end
...
temp=preambleRX;
clear preambleRX;
preambleRX=zeros(128,d.RXAntenna);
%Remove cylic prefix
for i=1:d.RXAntenna
    preambleRX(:,i)=temp(d.cyclicPrefixPreamble+1:end,i);
end
clear temp;
estimatedCFO=zeros(1,d.RXAntenna);
for i=1:d.RXAntenna
    estimatedCFO(1,i)=CFOPreamble(preambleRX(1:128,i));
    %CFO Correction
    frameR(:,i)=CFOCorrection(frameR(:,i),estimatedCFO(1,i));
    preambleRX(1:d.preambleLength,i)=frameR(1:d.preambleLength,i);
    dataRX(1:d.dataLength,i)=frameR(d.preambleLength+1:end,i);
end
...
%Tracking CFO
for i=1:d.RXAntenna
    dataRX(:,:,i)=CFOTracking(dataRX(:,:,i));
end

%Pilots removal
for i=1:d.RXAntenna
    dataRX(:,:,i)=pilotDetection(dataRX(:,:,i));
end
