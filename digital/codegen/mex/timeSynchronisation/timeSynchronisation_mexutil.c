...
const mxArray *b_emlrt_marshallOut(const struct_T *u)
{
  ...
  emxEnsureCapacity((emxArray__common *)e_u, i4, (int32_T)sizeof(creal_T),
                    (emlrtRTEInfo *)NULL);
  loop_ub = u->referenceSymbol->size[0];
  for (i4 = 0; i4 < loop_ub; i4++) {
    e_u->data[i4] = u->referenceSymbol->data[i4];
  }

  e_y = NULL;
  m5 = mxCreateNumericArray(1, e_u->size, mxDOUBLE_CLASS, mxCOMPLEX);
  emlrtExportNumericArrayR2013a(emlrtRootTLSGlobal, m5, (void *)e_u->data, 8);
  emlrtAssign(&e_y, m5);
  ...
  for (i4 = 0; i4 < loop_ub; i4++) {
    f_u->data[i4] = u->referenceSymbolMatrix->data[i4];
  }
  ...
  for (i4 = 0; i4 < loop_ub; i4++) {
    g_u->data[i4] = u->maskData->data[i4];
  }
  ...
  for (i4 = 0; i4 < loop_ub; i4++) {
    h_u->data[i4] = u->maskPilots->data[i4];
  }
